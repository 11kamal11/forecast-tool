...
def preprocess_time_series(df, date_col, target_col, freq='M'):
    try:
        ...
        if df[target_col].isna().any():
            st.warning("Filling NaN values in target after resampling with mean.")
        ...
        if unique_dates < 2:
            st.error(f"Only {unique_dates} unique date(s) found after resampling. Need at least 2 for forecasting.")
            return None
...
def generate_forecast(df, date_col, target_col, horizon):
    try:
        ...
        if len(y_pred) == 0 or np.any(np.isnan(y_pred)):
            st.warning("Unable to compute metrics due to empty or invalid predictions. Displaying forecast plots only.")
        ...
        fig1.update_layout(
            title=f"Monthly Forecast with Confidence Intervals (RMSE: {rmse if rmse else 'N/A'}, MAE: {mae if mae else 'N/A'}, MAPE: {mape if mape else 'N/A'}%)",
            xaxis_title="Date",
        ...
        fig3 = go.Figure()
        fig3.add_trace(go.Scatter(x=forecast['ds'], y=forecast['trend'], mode='lines', name='Trend'))
        if len(processed_df) >= 12:
            fig3.add_trace(go.Scatter(x=forecast['ds'], y=forecast['yearly'], mode='lines', name='Yearly Seasonality'))
        fig3.update_layout(
        ...
        st.write("Monthly Forecast Sample (last 10 rows):")
...
def generate_insights(df, date_col, target_col):
    try:
        df = df.copy()
        df[date_col] = pd.to_datetime(df[date_col], errors='coerce')
        df = df.dropna(subset=[date_col])
        ...
        fig_line.update_layout(
            title=f"Monthly Trend of {target_col}",
            xaxis_title="Date",
            yaxis_title=target_col,
            template="plotly_white"
        )
        ...
        # Distribution Plot
        fig_box = px.box(df_resampled, y='mean', title=f"Monthly Distribution of {target_col}")
        ...
        st.write(f"**Monthly Insights Sample** (last 10 rows)")
        st.dataframe(insights_df.tail(10))
        st.download_button(
            f"Download Monthly Insights Report",
...
def main():
    ...
    st.markdown("Upload any CSV with a date column (e.g., 2022-09-01, 202209) and a numeric column to generate monthly forecasts and insights.")
